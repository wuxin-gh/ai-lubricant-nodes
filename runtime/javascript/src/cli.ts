...
export function createProgram(options: { exitOverride?: boolean } = {}): Command {
  ...
  program
    ...
    // Keep in sync with package.json "version". Read at build time is awkward
    // under ESM (no __dirname), and the node probes `agent-compose-runtime
    // --version` to report runtime_version - a hardcoded constant is fine here.
    .version("0.6.0")
  ...
  program
    .command("prompt")
    .requiredOption("--provider <provider>", "agent provider: codex, claude, gemini, opencode, or cursor")
