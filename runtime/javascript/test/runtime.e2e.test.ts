  ...
  Codex: vi.fn().mockImplementation(function Codex(options: unknown) {
    ...
    return {
      startThread: vi.fn((threadOptions: unknown) => {
        ...
        return makeThread("e2e-codex-thread");
      }),
      // The per-turn runner resumes the stored thread on the second turn, so the
      // mock must provide resumeThread returning a thread with the same id.
      resumeThread: vi.fn((id: string, threadOptions: unknown) => makeThread(id)),
    };
  }),
}));
...
describe("runtime JavaScript E2E", () => {
  it("preserves turn boundaries across interactive Codex messages", async () => {
    await withTempSession(async (root) => {
      ...
      // The turn options now include an AbortSignal for cancel-current-turn;
      // keep this assertion about the semantic boundary (two ordered inputs),
      // not the provider SDK's incidental option serialization.
      const transcript = String(terminal?.transcript || "")
        .split("\n")
        .filter(Boolean)
        .map((line) => JSON.parse(line) as { input?: string });
      expect(transcript.map((entry) => entry.input)).toEqual(["first", "second"]);
    });
  });
